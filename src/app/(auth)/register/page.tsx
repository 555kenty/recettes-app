    ...
    <div className="mt-2">
      <div className="flex gap-1 mb-1.5">
        {[0, 1, 2, 3].map((i) => (
          <div
            key={i}
            className={`h-1 flex-1 rounded-full transition-all duration-300 ${i < score ? colors[score - 1] : 'bg-canvas-200'}`}
          />
        ))}
      </div>
      <p className={`text-[11px] font-medium ${score <= 1 ? 'text-red-500' : score === 2 ? 'text-amber-500' : 'text-emerald-600'}`}>
        {labels[score]}
      </p>
    </div>
  );
...
export default function RegisterPage() {
  ...
  return (
    ...
    >
      <div className="mb-8">
        <h1 className="font-serif text-3xl font-bold text-stone-900 mb-1.5">Crée ton compte</h1>
        <p className="text-stone-500 text-sm">Rejoins la communauté. C'est gratuit.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <label className="block text-xs font-semibold text-stone-700 uppercase tracking-wider mb-2">Prénom / Pseudo</label>
          <input
            type="text"
            value={name}
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.11 }}>
          <label className="block text-xs font-semibold text-stone-700 uppercase tracking-wider mb-2">Mot de passe</label>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }}>
          <button type="submit" disabled={loading || success} className="btn-primary w-full py-3.5 text-base mt-2">
            {success ? (
              <><CheckCircle2 className="w-4 h-4" /> Compte créé !</>
            ) : loading ? (
