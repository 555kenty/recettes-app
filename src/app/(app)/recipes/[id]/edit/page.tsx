...
export default function EditRecipePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      setCategory(recipe.category ?? 'Plat principal');
      setTimeMinutes(recipe.timeMinutes?.toString() ?? '');
      setDifficulty(recipe.difficulty ?? 'Moyen');
      ...
      setIngredients(
        rawIngredients.length > 0
          ? rawIngredients.map((i: { name?: string; quantity?: string; unit?: string }) => ({
              name: i.name ?? '',
              quantity: i.quantity ?? '',
              unit: i.unit ?? '',
            }))
      ...
      setSteps(
        rawSteps.length > 0
          ? rawSteps.map((s: { description?: string } | string) =>
              typeof s === 'string' ? { description: s } : { description: s.description ?? '' },
            )
          : [{ description: '' }],
      );
  ...
  const addStep = () => setSteps((p) => [...p, { description: '' }]);
  const removeStep = (i: number) => setSteps((p) => p.filter((_, idx) => idx !== i));
  const updateStep = (i: number, value: string) =>
    setSteps((p) => p.map((s, idx) => idx === i ? { description: value } : s));
  ...
  if (forbidden) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <p className="text-stone-500">Vous n&apos;êtes pas l&apos;auteur de cette recette.</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-bold text-slate-800">Modifier la recette</h1>
          <button
            type="button"
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-100 space-y-4">
              ...
              <label className="block cursor-pointer">
                ...
                {currentImage ? (
                  <div className="relative rounded-xl overflow-hidden aspect-video">
                    ...
                    <button
                      type="button"
                      onClick={(e) => { e.preventDefault(); setImageFile(null); setImagePreview(null); setExistingImageUrl(null); }}
                  ...
                  <div className="aspect-video rounded-xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-2 hover:border-rose-300 hover:bg-rose-50 transition-colors">
                    ...
                    <span className="text-sm text-slate-400">Cliquer pour changer la photo</span>
                  </div>
                )}
              </label>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" value={timeMinutes} onChange={(e) => setTimeMinutes(e.target.value)} min="1" className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-rose-400 focus:outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Portions</label>
                <input type="number" value={servings} onChange={(e) => setServings(e.target.value)} min="1" className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-rose-400 focus:outline-none" />
              </div>
            </div>
            ...
            <div className="space-y-3">
              {ingredients.map((ing, i) => (
                <div key={i} className="flex gap-2 items-center">
                  <input type="text" value={ing.name} onChange={(e) => updateIngredient(i, 'name', e.target.value)} placeholder="Ingrédient" className="flex-1 px-3 py-2.5 rounded-xl border border-slate-200 focus:border-rose-400 focus:outline-none text-sm" />
                  <input type="text" value={ing.quantity} onChange={(e) => updateIngredient(i, 'quantity', e.target.value)} placeholder="Qté" className="w-20 px-3 py-2.5 rounded-xl border border-slate-200 focus:border-rose-400 focus:outline-none text-sm" />
                  <input type="text" value={ing.unit} onChange={(e) => updateIngredient(i, 'unit', e.target.value)} placeholder="Unité" className="w-20 px-3 py-2.5 rounded-xl border border-slate-200 focus:border-rose-400 focus:outline-none text-sm" />
